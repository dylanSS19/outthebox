...
$clave_fac = $_GET['clave'];
$idempresa = $_GET['empresa'];
$fechaFac = $_GET['fecha'];

if($_GET['clave'] == "" || $_GET['empresa'] == "" || $_GET['fecha'] == ""){


header("HTTP/1.1 403 Forbidden");

      echo '{"success": "false", "reason": "Datos requeridos no pueden estar vacios."}';
...
break;
 ...
 $facturas = api_facturacionModel::MdlCargarEstadoFacturas($table, $clave_fac, $idempresa, $fechaFac);
 echo '<pre>'; print_r($facturas); echo '</pre>';

$estadoFac = $facturas[0]["estado_factura"];
$fechaFac = $facturas[0]["fecha_estado"];
 header("HTTP/1.1 200 OK");

    echo '{"success": "true", "Estado": "'.$estadoFac.'", "Fecha":"'.$fechaFac.'"}';
...
// echo '<pre>'; print_r($values); echo '</pre>';
 ...
 $respuesta_hacienda = api_facturacioncontroller::EnviarApiFacturas($token, $archivo_formado, $clave, $tipo_cedula_receptor, $cedula_receptor, $fecha_factura_2, $tipo_cedula_emisor, $cedula_emisor);
...
$IdFactura = api_facturacioncontroller::GuardarDatosFactura($id_compania, $sucursal, $caja, $fecha_creacion_factura, $fecha_creacion, $cancelado, $consecutivo_hacienda, $clave_hacienda, $tipeDoc, $actividaEconomica,$condicionVenta, $cedula, $nombre , $correo , $tipoCambio, $moneda);
...
$DetalleFactura = api_facturacioncontroller::GuardarDetalleFactura($IdFactura, $codigo, $nombre, $cantidad, $precio_unidad, $subtotal, $descuento, $impuesto, $total, $costo);
