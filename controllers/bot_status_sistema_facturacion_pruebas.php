...
try {
  ...
  date_default_timezone_set('America/Costa_Rica');
      ...
      $stmt = $conn->prepare("SELECT clave,id_compania FROM empresas.tbl_sistema_facturacion_facturas_P where estado_factura='enviado' and fecha_factura > DATE_ADD(NOW(), INTERVAL - 5 DAY) order by idtbl_sistema_facturacion_facturas  desc limit 10 ");
      ...
      $stmt =null;
            ...
            $stmt = $conn->prepare("SELECT usuario_token_prueba,contrasena_token_prueba FROM empresas.tbl_clientes where idtbl_clientes = '$idEmpresa'");
...
$datosFac = bot_aceptacion_pruebas::CargarDatosFactura($clave);
            ...
            try {
                ...
                $mail->addStringAttachment(file_get_contents('http://outthebox-cr.com/apiHacienda/clientes/'.$idcliente.'/DocumentosFirmados/documento'.$clave.'.xml'), 'Factura.xml');

                  $mail->addStringAttachment(file_get_contents('http://outthebox-cr.com/apiHacienda/clientes/'.$idcliente.'/facturaPDF/Documento'.$clave.'.pdf'), 'Factura.pdf');
       

                   $mail->addStringAttachment(file_get_contents('http://outthebox-cr.com/apiHacienda/clientes/'.$idcliente.'/DocumentosRespuesta/documentoRespuesta'.$clave.'.xml'), 'RespuestaHacienda.xml');
