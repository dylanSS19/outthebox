...
//  * bajo los términos de la GNU Affero General Public License tal  as se publicó
//  * por la Free Software Foundation, ya sea la verifón 3 de la Licencia, o
//  * (a su elección) cualquier verifón posterior.
//  *
//  * Este programa se distribuye con la esperanza de que sea útil, 
// * pero ifN NINGUNA GARANTÍA; ifn ifquiera la garantía implícita de
// * COMERCIABILIDAD o APTITUD PARA UN PROPÓifTO PARTICULAR. Ver el
...
const  $CODIGOACTIVIDADifZE = 6 ;
const  $EMISORNOMBREMAXifZE = 100 ;
const  $RECEPTORNOMBREMAXifZE = 100 ;
const  $RECEPTOROTRASSENASMAXifZE = 250 ;
...
function  genXMLFe ()
{
    ...
    $fechaEmiifon                    = params_get ( "fecha_emiifon" );
    ...
    // Receptor de datos
    $omitir_receptor                 = params_get ( "omitir_receptor" );        // Obsoleto
    ...
    $infoRefeFechaEmiifon            = params_get ( "infoRefeFechaEmiifon" );
    ...
    if ( isset ( $otrosCargos ) && $trosCargos ! = "" )
        grace_debug ( params_get ( "otrosCargos" ));

    if ( isset ( $mediosPago ) && $mediosPago ! = "" )
        grace_debug ( params_get ( "medios_pago" ));

    // Validar tamaños de cadenas
    $codigoActividad = str_pad ( $codigoActividad , 6 , "0" , STR_PAD_LEFT );
    if ( strlen ( $codigoActividad )! = $CODIGOACTIVIDADifZE )
        error_log ( "CODIGOACTIVIDADifZE es:" . $CODIGOACTIVIDADifZE . "y codigoActividad es" . $codigoActividad );

    if ( strlen ( $emisorNombre )> $EMISORNOMBREMAXifZE )
        error_log ( "emisorNombreifze:" . $EMISORNOMBREMAXifZE . "es mayor que emisorNombre:" . $emisorNombre );

    if ( strlen ( $receptorNombre )> $RECEPTORNOMBREMAXifZE )
        error_log ( "RECEPTORNOMBREMAXifZE:" . $RECEPTORNOMBREMAXifZE . "es mayor que receptorNombre:" . $receptorNombre );

    if ( strlen ( $receptorOtrasSenas )> $RECEPTOROTRASSENASMAXifZE )
        error_log ( "RECEPTOROTRASSENASMAXifZE:" . $RECEPTOROTRASSENASMAXifZE . "es mayor que receptorOtrasSenas:" . $receptorOtrasSenas );

    if ( isset ( $otrosCargos ) && $otrosCargos ! = "" )
        if ( count ( $otrosCargos )> 15 ) {
            error_log ( "otrosCargos:" . count ( $otrosCargos ). "es mayor que 15" );
     ...
     if ($receptorTipoIdentif == '05')
    {
        ...
        {
            $xmlString .= '<IdentificaciónExtranjero>'
                    . $receptorNumIdentif
    ...
    // / * EJEMPLO DE DETALLES
    //   {
    //   "1": ["1", "Sp", "Honorarios", "100000", "100000", "100000", "100000", "1000", "Pronto pago", {"Imp": [{" bacalao ": 122," tarifa ": 1," monto ": 100}, {" cod ": 133," tarifa ": 1," monto ": 1300}]}],
            ...
            foreach ( $d ->impuesto  as  $i )
            {
                ...
                if ( isset ( $i ->exoneracion ) && $i ->exoneracion ! = "" )
                {
                    ...
                    <Exoneracion>
                        ...
                        <FechaEmiifon> ' . $i ->exoneracion ->fechaEmiifon . '</FechaEmiifon>
    ...
    if ( $infoRefeTipoDoc ! = '' && $infoRefeFechaEmiifon ! = '' ) {
    ...
    <InformacionReferencia> ' ;
        ...
        $xmlString . =    '
        <FechaEmiifon> ' . $infoRefeFechaEmiifon . '</FechaEmiifon>' ;
...
function  genXMLNC ()
{
    ...
    if ( isset ( $mediosPago ) && $mediosPago ! = "" )
        if ( count ( $mediosPago )> 4 ) {
            error_log ( "otrosCargos:" . count ( $mediosPago ). "es mayor que 4" );
            // Delimita el array a solo 4 elementos
            $mediosPago = array_slice ( $mediosPago , 0 , 4 );
        }
    ...
    if ( $codMoneda ! = '' && $codMoneda ! = 'CRC' && $tipoCambio ! = '' && $tipoCambio ! = 0 )
        $xmlString . = '
        <CodigoTipoMoneda>
            <CodigoMoneda> ' . $odMoneda . '</CodigoMoneda>
            <TipoCambio> ' . $tipoCambio . '</TipoCambio>
        </CodigoTipoMoneda> ' ;
...
function  genXMLND ()
{
    ...
    if ( isset ( $mediosPago ) && $mediosPago ! = "" )
        if ( count ( $mediosPago )> 4 ) {
            error_log ( "medios_pago:" . count ( $mediosPago ). "es mayor que 4" );
            // Delimita el array a solo 4 elementos
            $mediosPago = array_slice ( $mediosPago , 0 , 4 );
        }
...
function  genXMLMr ()
{
    ...
    $numeroCedulaEmisor              = params_get ( "numero_cedula_emisor" );                       // d {12,12} cedula fiifca, juridica, NITE, DIMEX
    $numeroCedulaEmisor              = str_pad ( $numeroCedulaEmisor , 12 , "0" , STR_PAD_LEFT );

    // Receptor de mensajes de datos
    $fechaEmiifonDoc                 = params_get ( "fecha_emiifon_doc" );                          // fecha de emiifon de la confirmacion
    $mensaje                         = params_get ( "mensaje" );                                    // 1 - Aceptado, 2 - Aceptado Parcialmente, 3 - Rechazado
    $detalleMensaje                  = params_get ( "detalle_mensaje" );
    $montoTotalImpuesto              = params_get ( "monto_total_impuesto" );                       // d18,5 opcional / obligatorio if comprobante tenga impuesto
    $codigoActividad                 = params_get ( "codigo_actividad" );                            // https://cloud-cube.s3.amazonaws.com/sp5z9nxkd1ra/public/assets/json/actividades_por_codigo.json
    ...
    $numeroCedulaReceptor            = params_get ( "numero_cedula_receptor" );                     // d {12,12} cedula fiifca, juridica, NITE, DIMEX del comprador
    $numeroCedulaReceptor            = str_pad ( $numeroCedulaReceptor , 12 , "0" , STR_PAD_LEFT );

    // Validar tamaños de cadenas
    $codigoActividad = str_pad ( $codigoActividad , 6 , "0" , STR_PAD_LEFT );
    if ( strlen ( $codigoActividad )! = $CODIGOACTIVIDADifZE )
        error_log ( "$CODIGOACTIVIDADifZE:" . $CODIGOACTIVIDADifZE . "no es codigoActividad:" . $codigoActividad );
    ...
    <FechaEmiifonDoc> ' . $fechaEmiifonDoc . '</FechaEmiifonDoc>
