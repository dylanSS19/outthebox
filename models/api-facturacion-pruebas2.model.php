...
class api_facturacionModel {
		...
		static public function MdlcargarUidadMedida($table) {
			...
			static public function MdlcargarDatosEmpresa($table, $id_empresa) {
				...
				static public function MdlInsertarDatosFactura($table, $id_compania, $sucursal, $caja, $fecha_factura, $fecha_creacion, $cancelado, $consecutivo_hacienda, $clave_hacienda, $tipeDoc, $actividaEconomica,$condicionVenta, $cedula, $nombre , $correo , $tipoCambio, $moneda, $tipo_cedula, $plazo, $clvRefencia, $mediopago, $api) {
			
			$db = Connexion::connect();

			$estado = "enviado";

			$stmt = $db->prepare("INSERT INTO  $table (id_compania, sucursal, caja, fecha_factura, fecha_creacion, cancelado, consecutivo, clave, tipo_documento, codigo_actividad, condicion_venta, tipo_personeria, cedula_cliente, nombre_cliente, correo_cliente, tipo_cambio, codigo_moneda, fecha_estado, estado_factura, plazo_credito, referencia, medios_pago, api) VALUES(:id_compania, :sucursal, :caja, :fecha_factura, :fecha_creacion, :cancelado, :consecutivo, :clave, :tipo_documento, :codigo_actividad, :condicion_venta, :tipo_personeria, :cedula_cliente, :nombre_cliente, :correo_cliente, :tipo_cambio, :codigo_moneda, :fecha_estado, :estado_factura, :plazo_credito, :referencia, :medios_pago, :api)");				
...
static public function MdlInsertarDetalleFactura($table, $IdFactura, $codigo, $nombre, $cantidad, $precio_unidad, $subtotal, $descuento, $impuesto, $total, $costo, $cabys, $tasa_impuesto, $codImpuesto, $cosTasaImp, $unidadM) {

				$stmt = Connexion::connect()->prepare("INSERT INTO  $table (id_factura, codigo, nombre, cantidad, precio_unidad, subtotal, descuento, impuesto, total, costo, cabys, tasa_Impuesto, codImpuesto, codTasaImp, unidadMedida) VALUES(:id_factura, :codigo, :nombre, :cantidad, :precio_unidad, :subtotal, :descuento, :impuesto, :total, :costo, :cabys, :tasa_Impuesto, :codImpuesto, :codTasaImp, :unidadMedida)");				
	...
	static public function MdlInsertarUltimoConsecutivo($table, $id_factura, $ultimo_consecutivo, $sucursal, $caja, $random) {

		$db = Connexion::connect();


// $ultimo_consecutivo = "221";
			$stmt = $db->prepare("INSERT IGNORE INTO  $table (ultimo_consecutivo, sucursal, caja, random) VALUES(:ultimo_consecutivo, :sucursal, :caja, :random)");				
	...
	static public function MdlcargarUltimoConsecutivo($table, $sucursal , $caja) {
		...
		$stmt = Connexion::connect()->prepare("SELECT ifnull(max(ultimo_consecutivo),0) FROM $table where sucursal = '$sucursal' and caja = '$caja'");				
