...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'POST':
        ...
        if (api_CorreoNuevoEmpleado::is_json(json_encode($data["fileContent"])) == "true"){
            $correoRRHH = $data["fileContent"]["correoRRHH"];
            
            $cedula = $data["fileContent"]["cedula"];
            $nombre = $data["fileContent"]["nombre"];
            $primerApellido = $data["fileContent"]["primerApellido"];
            $segundoApellido = $data["fileContent"]["segundoApellido"];
            $fechaIngreso = $data["fileContent"]["fechaIngreso"];
            $departamento = $data["fileContent"]["departamento"];
            $puesto = $data["fileContent"]["puesto"];

            api_CorreoNuevoEmpleado::generarCorreo($correoRRHH, $cedula, $nombre, $primerApellido, $segundoApellido, $fechaIngreso, $departamento, $puesto);
        }

}


class api_CorreoNuevoEmpleado {
     ...
     *                          *
     ****************************/
    public static function generarCorreo($correoRRHH, $cedula, $nombre, $primerApellido, $segundoApellido, $fechaIngreso, $departamento, $puesto) {
        $mail = new PHPMailer(true);
        date_default_timezone_set('America/Costa_Rica');
        $date = date("d/m/Y", strtotime($fechaIngreso));
            try {
                //Server settings
                ...
                $body = '
                    ...
                    <body>
                        <div style="font-family: Arial, Helvetica, sans-ser">
                            ...
                            <p>
                                Hacemos de su conocimiento que el señor(a) <b>'.$nombre.' '.$primerApellido.' '.$segundoApellido.'</b><br> con número de  
                                identificación <b>'.$cedula.'</b>, ha sido registrado exitosamente <br>en el departamento de <b>'.$departamento.'</b>, 
                                <br> en el puesto <b>'.$puesto.'</b> y con fecha de ingreso de <b>'.$date.'</b>.
                            </p>
                ...
                $mail->Subject = 'REGISTRO DE NUEVO EMPLEADO';
                $flags = ENT_HTML5;
