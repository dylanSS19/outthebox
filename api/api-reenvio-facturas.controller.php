...
$enviarCorreo = api_reenvio_facturascontroller::EnviarCorreo();
...
if (api_reenvio_facturascontroller::is_json(json_encode($data["fileContent"])) == "true"){
...
$validacionCredenciales = api_reenvio_facturascontroller::validarCredencialesUsuario($usuario, $contrasena, $cedula);



      if($validacionCredenciales[0] == 1){

        $TipoDocumento = $json_cliente["fileContent"]["datosFactura"]["tipoDoc"];
        ...
        if($TipoDocumento != "01" || $TipoDocumento != "04"){
             ...
             echo '{"success": "false", "reason": "TIPO DE DOCUEMENTO NO VALIDO EN LA TRANSACCIÓN."}';
      ...
      $datosUsario = api_reenvio_facturascontroller::cargarDatosUsuario($contrasena, $cedula);
    ...
    $fecha_factura = ''.$fecha_emision_factura.'T'.$hora_emision_factura.'-06:00';
      ...
      =  GENERAR EL XML PARA FIRMAR                =
       =============================================*/
      $xml_factura = api_reenvio_facturascontroller::GenerarXML($data,$fecha_factura,$datosUsario["idtbl_clientes"]);
...
// echo '<pre>'; print_r($values); echo '</pre>';
      ...
      $ruta = "dfhjdfhj";
       ...
       /*===========================================F==
        ...
        $contrasena = $datosUsario["contrasena_token"]; 
           ...
           $token = api_reenvio_facturascontroller::GenerarToken($user, $contrasena);
 ...
 $respuesta_hacienda = api_reenvio_facturascontroller::EnviarApiFacturas($token, $archivo_formado, $clave, $tipo_cedula_receptor, $cedula_receptor, $fecha_factura_2, $tipo_cedula_emisor, $cedula_emisor);
...
// $envioCorreo = api_reenvio_facturascontroller::EnviarCorreo($clave, $datosUsario["idtbl_clientes"]);
...
class api_reenvio_facturascontroller{
...
$datosUsario = api_reenvio_facturascontroller::cargarDatosUsuario($contrasena, $cedula_emisor);
...
$cedula_formateada = api_reenvio_facturascontroller::generarCedula12Digitos ($datosUsario["cedula"]);
$año = date('y');
$mes = date('m');
$dia = date('d');
...
$IdFactura = api_reenvio_facturascontroller::GuardarDatosFactura($id_compania, $sucursal, $caja, $fecha_creacion_factura, $fecha_creacion, $cancelado, $consecutivo_hacienda, $clave_hacienda, $tipeDoc, $actividaEconomica,$condicionVenta, $cedula, $nombre , $correo , $tipoCambio, $moneda);
...
$TotalVentaNeta = 0;
$unidadMedida = array ();
$medidas = api_reenvio_facturascontroller::cargarUidadMedida();
...
$archivo_XML .=  '   <LineaDetalle>
  ...
  $archivo_XML .='
        ...
        <Tarifa>'.str_replace ( '"' , '' ,number_format(floatval(json_encode($value["tasaImpuesto"])), 5)).'</Tarifa>
...
$DetalleFactura = api_reenvio_facturascontroller::GuardarDetalleFactura($IdFactura, $codigo, $nombre, $cantidad, $precio_unidad, $subtotal, $descuento, $impuesto, $total, $costo);
...
$ModificarDatos = api_reenvio_facturascontroller::ModificarDatosFactura($TotalVentaNeta, $total_descuento_new, $total_impuesto_new, $otros_cargos, $TotalComprobante, $IdFactura);
...
$datosFac = api_reenvio_facturascontroller::CargarDatosFactura($clave);
    ...
    public function GuardarDatosFactura($id_compania, $sucursal, $caja, $fecha_factura, $fecha_creacion, $cancelado, $consecutivo_hacienda, $clave_hacienda, $tipeDoc, $actividaEconomica,$condicionVenta, $cedula, $nombre , $correo , $tipoCambio, $moneda){
       ...
       $insertFactura = api_facturacionModel::MdlInsertarDatosFactura($table, $id_compania, $sucursal, $caja, $fecha_factura, $fecha_creacion, $cancelado, $consecutivo_hacienda, $clave_hacienda, $tipeDoc, $actividaEconomica,$condicionVenta, $cedula, $nombre , $correo , $tipoCambio, $moneda);
   ...
   public function GuardarDetalleFactura($IdFactura, $codigo, $nombre, $cantidad, $precio_unidad, $subtotal, $descuento, $impuesto, $total, $costo){
        ...
        $insertDetalleFactura = api_facturacionModel::MdlInsertarDetalleFactura($table, $IdFactura, $codigo, $nombre, $cantidad, $precio_unidad, $subtotal, $descuento, $impuesto, $total, $costo);
  ...
  public function ValidarNodosJson($json){
            ...
            header("HTTP/1.1 403 Forbidden");
            echo "Error el archivo no cuenta con el formato Json correcto en el indice: fileContent";
            exit();
            }
            ...
            header("HTTP/1.1 403 Forbidden");
            echo "Error el archivo no cuenta con el formato Json correcto en el indice: fileContent{datosEmisor} ";
            exit();
            }
            ...
            header("HTTP/1.1 403 Forbidden");
            echo "Error el archivo no cuenta con el formato Json correcto en el indice: fileContent{datosReceptor}";
            exit();
            }
            ...
            header("HTTP/1.1 403 Forbidden");
            echo "Error el archivo no cuenta con el formato Json correcto en el indice: fileContent{datosFactura}";
            exit();
            }
            ...
            header("HTTP/1.1 403 Forbidden");
            echo "Error el archivo no cuenta con el formato Json correcto en el indice: fileContent{datosFactura:[detalleFactura]}";
            exit();
  ...
  public function ValidarCajaSucursal($json){
    ...
    $table = "empresas.tbl_sucursal_".$id_empresa;
        ...
        if(!$respuestaS){
                    ...
                    header("HTTP/1.1 403 Forbidden");
                    echo "Datos de la Sucursal no existentes en sistema.";
                    exit();
            ...
            if(!$respuestaS){
                    ...
                    header("HTTP/1.1 403 Forbidden");
                    echo "Datos de la caja no ligados a la sucursal o datos no existentes.";
                    exit();
