...
$validacionCredenciales = api_facturacioncontroller::validarCredencialesUsuario($usuario, $contrasena, $cedula);
      ...
      $datosUsario = api_facturacioncontroller::cargarDatosUsuario($contrasena, $cedula);
           ...
           if(!array_key_exists('access_token', $token)){
                ...
                echo '{"success": "false", "reason": "CREDENCIALES DE TOKEN INCORRECTAS", "user": "'.$datosUsario["usuario_token"].'", "contrasena": "'.$datosUsario["contrasena_token"].'"}';
...
$json_factura = '{
  ...
  "comprobanteXml":"'.$archivo_formado.'"
   ...
   $ch = curl_init("https://api.comprobanteselectronicos.go.cr/recepcion/v1/recepcion"); //ambiente produccion
...
$tipo = "FC";
// echo '<pre>'; print_r($tipeDoc); echo '</pre>';
...
//   api_facturacioncontroller::EliminarUltConsecutivoFC($sucursal, $caja, $tipo, $id_empresa);
...
<NombreComercial>'.$nomClient.'</NombreComercial>
<Ubicacion>
...
$datosFac = api_facturacioncontroller::CargarDatosFactura($clave);
  ...
  public function ValidarCajaSucursal($json){
    ...
    public function EliminarUltConsecutivoFC($sucursal, $caja, $tipo, $empresa){

        $table = 'empresas.tbl_ultimo_consecutivo';
  
        $DatosEmpresa = api_facturacionModel::MdlEliminarUltConsecutivoFC($table, $sucursal, $caja, $tipo, $empresa);
