        ...
        $contrasena = $datosUsario["contrasena_token"]; 
          ...
          if(!isset($token->{'access_token'})){
            ...
            // if(!array_key_exists('access_token', $token)){
              ...
              api_facturacioncontroller::GenerarPdf($clave, $usuario);
              header("HTTP/1.1 403 Forbidden");

                echo '{"success": "false", "reason": "CREDENCIALES DE TOKEN INCORRECTAS" , "Clave": "'.$clave.'" , "Consecutivo": "'.$consecutivo.'"}';
...
$cabys = $value["cabys"];
$codImpuesto = $value["codTasaImpuesto"];
$cosTasaImp = $value["tipoImpuesto"];
...
$datosFac = api_facturacioncontroller::CargarDatosFactura($clave);
  ...
  public function ValidarCajaSucursal($json){
    ...
    public function ModificarEstadoAnulacion($clave, $estadoAnulacion){
        ...
        public function GenerarPdf($clave, $empresa){
          ...
          $data = '{
            ...
            "idcliente":"'.$empresa.'"        
              ...
              // $data = "client_id=api-stag&username=".$user."&password=".$contrasena."&grant_type=password"; 
         $ch = curl_init("http://backup.midigitalsat.com/api/api_crea_pdf.php"); 
            // $ch = curl_init("https://idp.comprobanteselectronicos.go.cr/auth/realms/rut-stag/protocol/openid-connect/token"); //ambiente pruebas
           //$ch = curl_init("https://posfacturar.com/pos_digitalsat/public/api/v5/sale/getBillSearch");
